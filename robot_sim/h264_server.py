...
baseline H.264 (Annex B), piping the raw bytes straight to the socket.
    ...
    python robot_sim/h264_server.py /dev/video0 1280 720 15 49601  # Linux v4l2 device
...
CAM = sys.argv[1] if len(sys.argv) > 1 else ("0" if sys.platform == "darwin" else "/dev/video0")
...
def main():
    ...
    s.bind(("0.0.0.0", PORT))
    s.listen(1)
    print(f"h264 server listening on tcp/{PORT} (cam {camera_device()} {WIDTH}x{HEIGHT}@{FPS})", flush=True)
    while True:
        conn, addr = s.accept()
